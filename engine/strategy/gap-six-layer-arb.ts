...
type BotMode = "IDLE" | "PURE_ARB" | "GAP_FIRST_LEG" | "WAIT_HEDGE" | "LOCKED" | "RESCUE";
...
function lockTarget(firstPrice: number, cfg: HybridConfig): number {
  return Number((1 - firstPrice - cfg.minLockEdgeCents / 100).toFixed(4));
}
...
function pushStatus(params: {
  ...
  ctx.setDiagnostics({
    pipeline: pipeline({
      ...
      validate: {
        ...
        detail: firstLeg
          ...
          : `First-leg <= ${cfg.firstLegMaxPrice.toFixed(2)}, khóa lời >= ${cfg.minLockEdgeCents}¢`,
      },
    ...
    arb: {
      ...
      hedgeMaxPrice,
      lastAction: detail,
      riskNote: "Bản hybrid chỉ khóa lời khi first-leg + hedge-leg cùng size và tổng giá < 1.00. Nếu chưa hedge được thì vẫn còn rủi ro 1 phe.",
    },
  });
}
...
export const gapSixLayerArb: Strategy = async (ctx) => {
  ...
  pushStatus({
    ctx,
    cfg,
    detail: "Hybrid GAP + 6-Layer đã sẵn sàng",
    trades: cycles,
  ...
  const timer = setInterval(() => {
    ...
    if (firstLeg && firstLeg.status === "filled" && firstLeg.filledShares > firstLeg.hedgeShares) {
      ...
      const lockQty = smartLockOk ? hedgeQty(needShares, hedgeDepth, cfg, true) : 0;
      const rescueQty = canRescueByPrice ? hedgeQty(needShares, hedgeDepth, cfg, true) : 0;
      ...
      ctx.setDiagnostics({
        pipeline: pipeline({
          ...
          validate: {
            state: lockQty > 0 ? "pass" : canLockByPrice ? "warn" : "watch",
            detail: `${prettySide(hedgeSide)} ask ${fmtPrice(hedgeAsk?.price)} cần <= ${maxLock.toFixed(2)} | đáy=${lowest?.toFixed(2) ?? "--"} | ${smartTrigger}`,
          },
          size: { state: lockQty > 0 || rescueQty > 0 ? "pass" : "watch", detail: `Cần hedge ${needShares.toFixed(2)}, có thể bắn ${Math.max(lockQty, rescueQty).toFixed(2)}` },
          fill: { state: "pass", detail: `First ${prettySide(firstLeg.side)} @ ${avgFirst.toFixed(2)}` },
          hedge: {
            state: lockQty > 0 ? "pass" : rescueQty > 0 ? "warn" : "watch",
            detail: lockQty > 0
        ...
        arb: {
          mode: mustRescue ? "RESCUE" : "WAIT_HEDGE",
          ...
          riskNote: "Đang giữ 1 phe: chưa phải arbitrage. Smart hedge sẽ đợi giá phe còn lại rơi sâu, rồi bắn khi giá siêu rẻ hoặc có dấu hiệu hồi từ đáy; gần hết giờ vẫn rescue theo MAX_RESCUE_SUM.",
    ...
    if (remaining < cfg.minRemainingSecs) {
      ctx.setDiagnostics({
        ...
        arb: { mode: "IDLE", trades: cycles, lastAction: "Ngừng entry gần close" },
      });
      clearInterval(timer);
      release();
      return;
    ...
    const pureValid = cfg.pureArbEnabled && pureSum !== null && pureEdge !== null && pureSum <= cfg.maxEntrySum && pureEdge >= cfg.minLockEdgeCents && pureDepthOk;
    ...
    if (pureValid) {
      ...
      ctx.setDiagnostics({
        pipeline: pipeline({
          ...
          detect: { state: "pass", detail: `Pure arb SUM=${pureSum!.toFixed(4)}` },
        ...
        arb: {
          ...
          lastTrigger: `BUY ${upAsk.price.toFixed(2)} + SELL ${downAsk.price.toFixed(2)} = ${pureSum!.toFixed(4)}`,
        },
      });
    ...
    const withinEntryWindow = openedFor <= cfg.firstEntryWindowSecs;
    ...
    if (!canUseGap) {
      ...
      if (pureSum !== null && pureSum > cfg.maxEntrySum) reasons.push(`Pure SUM ${pureSum.toFixed(4)} > max ${cfg.maxEntrySum.toFixed(4)}`);

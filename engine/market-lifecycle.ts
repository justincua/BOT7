...
function makeArbSnapshot(params: {
  ...
  const minEdgeCents = readNumEnv("ARB_MIN_EDGE_CENTS", 1);
  const maxEntrySum = readNumEnv("ARB_MAX_ENTRY_SUM", Number((1 - minEdgeCents / 100).toFixed(4)));
  ...
  const executable = sum !== null && sum <= maxEntrySum && (depthShares ?? 0) >= targetShares;
